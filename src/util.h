...
namespace Util {
  ...
  public:
    ...
    //.../folder/xxx.yy-->folder
    static QString FolderName(const QString& strFullPath)
    {
      QString strPath(strFullPath);
      strPath.replace('\\','/');
      int slashPos=strFullPath.lastIndexOf('/');
      QString strPathFolder =strPath.mid(0,slashPos);
      slashPos=strPathFolder.lastIndexOf('/');
      return strPathFolder.mid(slashPos+1);
        }
    ...
    static bool IsEmptyFile(const QString& strFile)
    {
      QString strContent=FileContent(strFile);
  ...
  public:
    ...
    static QString ReadSetting(const QString &key){
      ...
      QSettings settings(gCorpName,gAppName);
      return settings.value(key).toString();
    }

    static QByteArray ReadSetting2ByteA(const QString &key){
    ...
    //writeSetting
    static void WriteSetting2ByteA(const QString &key, const QByteArray &value){
  ...
  public:
    ...
    static void CenterWindow(QWidget* target)
    {
      ...
      QPoint ScreenCenter = desktop.screenGeometry().center();
      int wndX=target->width()/2;
      int wndY=target->height()/2;
      QPoint movePoint(ScreenCenter.x()-wndX/2,ScreenCenter.y()-wndY/2);
